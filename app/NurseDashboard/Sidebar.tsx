...
const Sidebar: React.FC<SidebarProps> = ({ onOptionClick }) => {
        ...
        const sideItems = [
            { id: 1, text: "Surgeries", icon: faSyringe },
            { id: 2, text: "Schedule ", icon: faCalendarDays }
          ];

    return (
        <>
        <div className="bg-[#003049] flex flex-col w-[95%] md:w-[15%] px-2 md:px-4 md:p-6 text-white z-10 space-y-4 md:space-y-8  rounded-lg  m-auto mt-36 md:mt-44 ml-2 md:ml-8 h-16 md:h-[50%]">   
        {/* Logo */}
        <div className="flex items-center ">
          <h1 className="hidden md:block text-3xl text-[#fdf0d5] pacifico-font">Workspace</h1>
        </div>
          ...
          {sideItems.map((item) => (
            <li key={item.id}>
              <button
                ...
                className="hover:text-[#f2d7bc] font-bold text-md flex items-center text-white"
              >
        ...
        <div onClick={handleSidebar} className="block md:hidden h-0">
          ...
          <h1 className="w-full text-2xl font-bold text-[#fdf0d5] pacifico-font flex items-center">Workspace</h1>
        ...
        {/* Mobile Logo */}
          <div className='flex items-center m-4'>
            <h1 className="w-full text-2xl font-bold text-[#fdf0d5] pacifico-font">Workspace</h1>
          ...
          {sideItems.map((item) => (
            <li key={item.id} className="p-4 border-b rounded-xl hover:bg-[#f2d7bc] duration-300 hover:text-black cursor-pointer border-gray-600 flex items-center">
              <button onClick={() => onOptionClick(item.text)}>
                <span>
